...
- [X] T004 [P] Create `/home/admin1/ovoco/kit-validator/lib/load-data.js`: exports `loadData(loadedSchema)` returning `LoadedData` per `data-model.md`. For each type in `loadedSchema.types`, computes the kebab-case data file name and reads `data/<kebab>.json` if present. Reads each data file as a flat JSON array of records. Stores per-type record arrays. Records missing data files in `loadedData.files` as null entries.
...
- [X] T029 Tag and push v0.1.0: `git tag v0.1.0 && git push origin v0.1.0`. This is the moment hr-kit's `npm install` of `git+https://github.com/ovoco-co/kit-validator.git#v0.1.0` becomes resolvable.
