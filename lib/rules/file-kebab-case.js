'use strict';

const fs = require('fs');
const path = require('path');

const KEBAB = /^[a-z][a-z0-9]*(-[a-z0-9]+)*\.json$/;

module.exports = function fileKebabCase(ctx) {
  const records = [];
  // Check filenames across the primary data directory and every domain
  // data directory, so a non-kebab filename in any of them is flagged.
  const dataDirs = Array.isArray(ctx.data.dataDirs) ? ctx.data.dataDirs : [ctx.data.dataDir];
  for (const dataDir of dataDirs) {
    if (!dataDir || !fs.existsSync(dataDir)) continue;

    const entries = fs.readdirSync(dataDir);
    for (const name of entries) {
      if (!name.endsWith('.json')) continue;
      if (!KEBAB.test(name)) {
        records.push({
          message: `Data file name '${name}' is not kebab-case.`,
          file: relPath(ctx, path.join(dataDir, name)),
          recordName: null,
          field: null,
        });
      }
    }
  }
  return records;
};

function relPath(ctx, abs) {
  const sd = ctx.schema.schemaDir;
  if (abs && abs.startsWith(sd)) return abs.slice(sd.length).replace(/^\//, '');
  return abs;
}
